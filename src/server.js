...
app.get("/:folderName", async (req, res) => {
  ...
  try {
    ...
    res.send(
      `Created and updated Pinecone index for ${folderName}: ${indexName}`
    );
  } catch (error) {
    // Folder doesn't exist, create a new Pinecone index
    console.log("error creating and update pinecone", error);
    res.send(
      `Error creating and updating Pinecone index for ${folderName}: ${indexName}`
    );
  }
});
...
//   // Determine indexName based on storeName
//   const indexName = storeName + "-index";
//   console.log("indexName", indexName);

//   const vectorDimension = 1536;
//   const loader = new DirectoryLoader(`./documents/${storeName}`, {
//     ".json": (path) => new JSONLoader(path, "/texts"),
//     ".jsonl": (path) => new JSONLinesLoader(path, "/html"),
//     ".txt": (path) => new TextLoader(path),
//     ".csv": (path) => new CSVLoader(path, "text"),
...
//     res.send(
//       `Created and updated Pinecone index for ${storeName}: ${indexName}`
//     );
//   } catch (error) {
//     // Folder doesn't exist, create a new Pinecone index
//     console.log("error creating and update pinecone", error);
//     res.send(
//       `Error creating and updating Pinecone index for ${storeName}: ${indexName}`
//     );
//   }
// });
...
app.post("/:storeName", async (req, res) => {
  ...
  // console.log("question server", question);
  const storeName = req.params.storeName;
  const indexName = storeName + "-index";
    // console.log("indexName server", indexName);
